                ...
                @forelse ($event_types as $event_type)
                    <div class="card">
                        <div class="card-header bg-transparent">
                            ...
                            <a href="{{ route('event_types.edit', $event_type->id) }}" class="btn btn-sm">
                            ...
                            <form action="{{ route('event_types.destroy', $event_type->id) }}" method="post" class="d-inline">
                                @csrf
                                @method('DELETE')
                        ...
                        <div class="card-body">
                            @forelse ($event_type->fields as $event_field)
                                ...
                                @endforeach
                            @empty
                                <p>No fields for this Event Type</p>
                            @endforelse
                        </div>
                    </div>
                @empty
                    <h2>No Event Types Added</h2>
                @endforelse
            </div>
        </div>
    </div>
